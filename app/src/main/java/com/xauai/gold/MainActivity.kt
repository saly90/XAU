...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        handler.postDelayed(object : Runnable {
            override fun run() {
                fetchSpot()
                if (System.currentTimeMillis() - lastHistoryFetch > 30000L) fetchHistory()
    ...
    private fun buildUi() {
        ...
        val live = TextView(this).apply {
            text = "  ● STREAM"; textSize = 10f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.rgb(65,225,150))
        }
        header.addView(title); header.addView(live)
        val refresh = TextView(this).apply {
            text = "↻"; textSize = 27f; gravity = Gravity.CENTER; setTextColor(Color.WHITE)
            setOnClickListener { fetchHistory(); fetchSpot() }
        }
        header.addView(refresh, LinearLayout.LayoutParams(dp(42), dp(42)).apply { gravity = Gravity.RIGHT })
        ...
        priceRow.addView(changeText, LinearLayout.LayoutParams(dp(85), dp(43)))
        root.addView(priceRow)
        statusText = TextView(this).apply { text = "در حال اتصال..."; textSize = 11f; setTextColor(Color.rgb(145,160,178)) }
        ...
        addPreset(presets, "SCALP", "1/2/5m")
        addPreset(presets, "INTRADAY", "5/15/1H")
        addPreset(presets, "SWING", "1H/4H")
        root.addView(presets, LinearLayout.LayoutParams(-1, dp(34)))
        ...
        listOf(1,2,3,5,15,30,60).forEach { m ->
            val b = TextView(this).apply {
                ...
                background = rounded(if (m == timeframe) Color.rgb(44,76,108) else Color.rgb(17,24,34), 9)
                setOnClickListener { timeframe = m; refreshTfButtons(); updateAnalysis() }
            }
            b.tag = m
            tfRow.addView(b, LinearLayout.LayoutParams(0, dp(34), 1f).apply { setMargins(dp(1),0,dp(1),0) })
        }
        root.addView(tfRow)
        ...
        val signalCard = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL; setPadding(dp(12),dp(7),dp(12),dp(7)); background = rounded(Color.rgb(14,20,29),15)
        }
        val signalRow = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL }
        signalText = TextView(this).apply { text="WAIT"; textSize=21f; typeface=Typeface.DEFAULT_BOLD; setTextColor(Color.rgb(240,190,75)) }
        signalRow.addView(signalText, LinearLayout.LayoutParams(0, dp(31), 1f))
        val paper = TextView(this).apply { text="PAPER • AI FILTER"; textSize=9f; setTextColor(Color.rgb(150,165,182)); gravity=Gravity.CENTER_VERTICAL }
        signalRow.addView(paper)
        signalCard.addView(signalRow)
        levelsText = TextView(this).apply { text="Entry —   SL —   TP1 —   TP2 —   TP3 —"; textSize=11f; setTextColor(Color.rgb(215,222,232)) }
        signalCard.addView(levelsText)
        statsText = TextView(this).apply { text="EMA 9/21 • RSI • ATR • PA • FIB • ICHI • MTF"; textSize=9f; setTextColor(Color.rgb(120,140,160)); setPadding(0,dp(3),0,0) }
        signalCard.addView(statsText)
        root.addView(signalCard, LinearLayout.LayoutParams(-1, dp(73)).apply { topMargin=dp(7) })
        setContentView(root)
    }

    private fun addPreset(row: LinearLayout, name: String, sub: String) {
        val b = TextView(this).apply {
            text = "$name  $sub"; textSize = 9f; gravity = Gravity.CENTER; setTextColor(Color.rgb(165,180,198)); background = rounded(Color.rgb(11,16,24),8)
        }
        b.setOnClickListener {
            timeframe = when(name) { "SCALP" -> 2; "INTRADAY" -> 5; else -> 60 }
            refreshTfButtons(); updateAnalysis()
        }
        row.addView(b, LinearLayout.LayoutParams(0,dp(30),1f).apply { setMargins(dp(2),0,dp(2),dp(2)) })
    }

    private fun refreshTfButtons() {
        for (i in 0 until tfRow.childCount) {
            val v = tfRow.getChildAt(i)
            val m = v.tag as Int
            v.background = rounded(if(m == timeframe) Color.rgb(44,76,108) else Color.rgb(17,24,34), 9)
        }
    }

    private fun fetchHistory() {
        statusText.text = "دریافت کندل‌های XAU/USD..."
        io.execute {
            try {
                val body = get("https://xaus.com/api/v1/intraday?symbol=xau&hours=12", 3500)
                val arr = JSONObject(body).optJSONArray("points") ?: throw Exception("no points")
                val raw = ArrayList<PointPrice>(arr.length())
                for (i in 0 until arr.length()) {
                    val p = arr.getJSONObject(i)
                    raw.add(PointPrice(p.optLong("t"), p.optDouble("p")))
                }
                if (raw.size < 40) throw Exception("not enough data")
                val built = aggregate(raw, timeframe)
                runOnUiThread {
                    candles = built
                    lastHistoryFetch = System.currentTimeMillis()
                    statusText.text = "XAU/USD • ${built.size} کندل • بروزرسانی خودکار"
                    updateAnalysis()
                }
            } catch (e: Exception) {
                runOnUiThread { statusText.text = "خطای شبکه؛ تلاش خودکار ادامه دارد" }
            }
        }
    }

    private fun fetchSpot() {
        io.execute {
            try {
                val body = get("https://xaus.com/api/v1/spot", 2200)
                val json = JSONObject(body)
                val p = findNumber(json, listOf("price","rate","value","mid"))
                if (p > 0) runOnUiThread { livePrice = p; applyLivePrice() }
            } catch (_: Exception) { }
        }
    }
    ...
    private fun findNumber(j: JSONObject, keys: List<String>): Double {
        for (k in keys) if (j.has(k)) {
            val v = j.optDouble(k, Double.NaN); if (!v.isNaN() && v > 0) return v
        }
        val names = j.keys()
        while (names.hasNext()) {
            val k = names.next(); val v = j.opt(k)
            if (v is JSONObject) { val n = findNumber(v, keys); if (n > 0) return n }
        }
        return 0.0
    }

    private fun applyLivePrice() {
        if (livePrice <= 0) return
        val old = if (candles.isNotEmpty()) candles.last().close else livePrice
        priceText.text = "XAU/USD  ${fmt(livePrice)}"
        val d = livePrice - old
        changeText.text = if (abs(d) < 0.005) "LIVE" else String.format(Locale.US,"%+.2f",d)
        changeText.setTextColor(if(d >= 0) Color.rgb(70,220,150) else Color.rgb(255,95,95))
        if (candles.isNotEmpty()) {
            val copy = candles.toMutableList(); val last = copy.last(); copy[copy.lastIndex] = last.copy(high=max(last.high,livePrice), low=min(last.low,livePrice), close=livePrice); candles=copy
            updateAnalysis()
        }
        statusText.text = "● LIVE PRICE  ${fmt(livePrice)}  •  Paper Trading"
    }

    private fun updateAnalysis() {
        if (candles.size < 30) return
        val close = candles.map { it.close }
        val e9 = ema(close,9); val e21 = ema(close,21); val e50 = ema(close,50); val r = rsi(close,14); val a = atr(candles,14)
        val recent = close.takeLast(8); val momentum = recent.last() - recent.first()
        val hh = candles.takeLast(30).maxOf { it.high }; val ll = candles.takeLast(30).minOf { it.low }
        val fib382 = hh-(hh-ll)*0.382; val fib618 = hh-(hh-ll)*0.618
        val trend = when { e9 > e21 && e21 > e50 -> 1; e9 < e21 && e21 < e50 -> -1; else -> 0 }
        val nearFib = abs(close.last()-fib382) < a*0.8 || abs(close.last()-fib618) < a*0.8
        val price = if(livePrice > 0) livePrice else close.last()
        val rawScore = 50.0 + trend*14.0 + if(momentum > 0) 8 else if(momentum < 0) -8 else 0.0 + if(r > 50) 5 else -5 + if(nearFib) 5 else 0.0
        val type = when { rawScore >= 68 && r < 74 -> "BUY"; rawScore <= 32 && r > 26 -> "SELL"; else -> "WAIT" }
        val conf = min(94, max(50, abs(rawScore-50).toInt()+50))
        val risk = max(a*1.15, price*0.00035)
        val sl = if(type=="SELL") price+risk else price-risk
        val tp1 = if(type=="SELL") price-risk else price+risk
        val tp2 = if(type=="SELL") price-risk*1.7 else price+risk*1.7
        val tp3 = if(type=="SELL") price-risk*2.4 else price+risk*2.4
        signal = Signal(type,price,sl,tp1,tp2,tp3,conf)
        priceText.text = "XAU/USD  ${fmt(price)}"
        ...
        levelsText.text = "Entry ${fmt(price)}   SL ${fmt(sl)}   TP1 ${fmt(tp1)}   TP2 ${fmt(tp2)}   TP3 ${fmt(tp3)}"
        statsText.text = "EMA ${fmt(e9)}/${fmt(e21)} • RSI ${r.toInt()} • ATR ${fmt(a)} • FIB ${fmt(fib382)}/${fmt(fib618)} • MTF"
        chart.data = candles; chart.signal = signal; chart.invalidate()
    }

    private fun aggregate(p: List<PointPrice>, m: Int): List<Candle> {
        val b = m * 60000L; val out=ArrayList<Candle>(); var bucket=-1L; var o=0.0; var h=0.0; var l=0.0; var c=0.0
        for(x in p) { val k=(x.t/b)*b; if(k!=bucket) { if(bucket>=0) out.add(Candle(bucket,o,h,l,c)); bucket=k;o=x.price;h=x.price;l=x.price;c=x.price } else { h=max(h,x.price); l=min(l,x.price); c=x.price } }
        if(bucket>=0) out.add(Candle(bucket,o,h,l,c)); return out
    }
    ...
    class GoldChartView(context:android.content.Context):View(context) {
        ...
        private fun drawMarker(c:Canvas,last:Candle,type:String,lo:Double,range:Double,left:Float,right:Float,top:Float,bottom:Float){val x=right-3;val price=last.close;val y=toY(price,lo,range,top,bottom);p.color=if(type=="BUY")Color.rgb(60,220,150)else Color.rgb(255,80,90);p.style=Paint.Style.FILL;val path=Path();if(type=="BUY"){path.moveTo(x,y-15);path.lineTo(x-10,y-2);path.lineTo(x+10,y-2)}else{path.moveTo(x,y+15);path.lineTo(x-10,y+2);path.lineTo(x+10,y+2)}path.close();c.drawPath(path,p)}
    }
}
